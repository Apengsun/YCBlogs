...
### 02.switch对整型支持
- 下面是一段很简单的Java代码，定义一个int型变量a，然后使用switch语句进行判断。执行这段代码输出内容为5，那么将下面这段代码反编译，看看他到底是怎么实现的。[博客](https://github.com/Apengsun/YCBlogs)
    ```
...
- 编译后的代码如下： `public class switchDemoChar
    ...
    ```
- 通过以上的代码作比较我们发现：对char类型进行比较的时候，实际上比较的是ascii码，编译器会把char型变量转换成对应的int型变量。[博客](https://github.com/Apengsun/YCBlogs)
